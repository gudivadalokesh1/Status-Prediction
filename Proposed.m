% Improved Electric Fish Optimization (IEFO)- starting to ending line n0.(35 - 38) 
function [bestPosition, Convergence_curve, bestFitness, time] = Proposed(fishPositions, LB, UB, objectiveFunction, numGenerations)
lb = LB(1, :);
ub = UB(1, :);
...
for generation = 1:numGenerations
    ...
    for i = 1:numFish
        bestfit=min(fishFitness);
        worstfit= max(fishFitness);
        meanfit = mean(fishFitness);
        r = bestfit / (worstfit * meanfit);
        % Random fish movement
        randomStep = r(1, numDimensions); randn(1, numDimensions);
